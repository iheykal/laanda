...
module.exports = socket => {
    ...
    const handleMovePawn = async data => {
        ...
        if (isMoveValid(req.session, pawn, room)) {
            ...
            if (getAnotherTurn) {
                if (movedWithSix) {
                    console.log('🎲 Bonus turn granted - moved with a 6!');
                }
                if (capturedOpponent) {
                    console.log('⚔️ Bonus turn granted - captured opponent pawn!');
                }
            }
            ...
            if (!getAnotherTurn) {
                ...
                console.log(`🔄 Player ${currentPlayer.color} turn ending - switching to next player`);
                room.rollHistory = [];
            ...
            // Save and broadcast room state IMMEDIATELY after turn change
            const updatedRoom = await updateRoom(room);
            if (updatedRoom) {
                sendToPlayersData(updatedRoom);
                console.log(`📤 Room data broadcasted after move - Next player: ${updatedRoom.getCurrentlyMovingPlayer()?.color}`);
            } else {
                console.error('❌ Failed to update room after move');
            }
        }
    };
    ...
    const handleRollDice = async () => {
        ...
        if (!player.canMove(room, rolledNumber)) {
            console.log(`⏰ Player ${player.color} (ID: ${player._id}) cannot move with roll ${rolledNumber} - ending turn and switching player`);
            room.rolledNumber = null;
            room.rollHistory = [];
            room.changeMovingPlayer();
            console.log(`✅ Turn switched after no valid moves`);
        }
        
        // Save and broadcast room state IMMEDIATELY after roll
        const updatedRoom = await updateRoom(room);
        if (updatedRoom) {
            sendToPlayersData(updatedRoom);
            const nextPlayer = updatedRoom.getCurrentlyMovingPlayer();
            console.log(`📤 Room data broadcasted after roll - Current player: ${nextPlayer?.color} (ID: ${nextPlayer?._id})`);
        } else {
            console.error('❌ Failed to update room after roll');
        }
    };
