...
RoomSchema.methods.changeMovingPlayer = function () {
    ...
    console.log(`🔄 Turn changed in room ${this._id}: ${oldPlayer?.color || 'unknown'} (ID: ${oldPlayer?._id}) → ${newMovingPlayer?.color || 'unknown'} (ID: ${newMovingPlayer?._id}, ${newMovingPlayer?.isBot ? 'BOT' : 'HUMAN'})`);
    console.log(`👥 All players in room: ${this.players.map(p => `${p.color}(${p._id}, nowMoving: ${p.nowMoving})`).join(', ')}`);
...
RoomSchema.methods.canStartGame = function () {
    // CRITICAL: Game can only start with exactly 2 players, both ready
    const readyCount = this.players.filter(player => player.ready).length;
    const hasTwoPlayers = this.players.length === 2;
    return readyCount >= 2 && hasTwoPlayers;
};

RoomSchema.methods.startGame = async function () {
    // SAFETY CHECK: Never start game with less than 2 players
    if (this.players.length < 2) {
        console.error(`❌ CANNOT START GAME: Only ${this.players.length} player(s) in room ${this._id}. Need 2 players!`);
        return false;
    }
    
    // Double check both players are present
    if (this.players.length !== 2) {
        console.error(`❌ CANNOT START GAME: Expected 2 players, found ${this.players.length} in room ${this._id}`);
        return false;
    }
    
    console.log(`✅ Starting game with ${this.players.length} players: ${this.players.map(p => p.name).join(', ')}`);
    this.started = true;
    this.nextMoveTime = Date.now() + MOVE_TIME;
    ...
    return true; // Return true to indicate game started successfully
};
